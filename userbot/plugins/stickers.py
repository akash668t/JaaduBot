...
async def _(event):
    ...
    #packname = f"JaadukeStickers"
    #packshortname = f"JaaduBot_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 948408212:
        packname = f"JaadukeStickers"
        packshortname = "Jaadu_ke_locker_me"
    else:
        packname = f"JaaduBot{userid}"
        packshortname = f"JaaduBot_{userid}_ns"
    ...
    if is_a_s:
        ...
        if userid == 948408212:
            packshortname = "Jaadu_Animated"
        else:
            packshortname = f"JaaduBot_{userid}_an" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                   ...
                   # else:
                       # packname = f"Vol._{pack}_JaaduBot{userid}"
                        #packshortname = f"Vol._{pack}_JaaduBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
